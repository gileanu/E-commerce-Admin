...
export default function StoreSwitcher({
    ...
    const currnetStore = formatedItems.find((item) => item.value === params.storeId)
                        ...
                        <CommandGroup heading="Stores">
                            {formatedItems.map((store) => (
                                <CommandItem key={store.value} onSelect={() => onStoreSelect(store)} className="text-sm">
                                    <StoreIcon className="mr-2 h-4 w-4"/>
                                    {store.label}
                                    <Check className={cn("ml-auto h-4 w-4", currnetStore?.value === store.value ? "opacity-100" : "opacity-0'")} />
                                </CommandItem>
                            ))}
                        </CommandGroup>
                        </CommandInput>
